...
const AddProduct = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!image) {
      setAlertState({ show: true, type: 'warning', text: 'Please select an image' });
      return;
    }
    ...
    try {
      ...
      if (res.ok) {
        setAlertState({ show: true, type: 'success', text: 'Product created successfully with Cloudinary Image URL!' });
        navigate('/shop');
      } else {
        setAlertState({ show: true, type: 'error', text: responseData.message || 'Error creating product' });
      }
    } catch (error) {
      console.error(error);
      setAlertState({ show: true, type: 'error', text: 'Something went wrong while creating the product.' });
    } finally {
      setLoading(false);
    }
  };
